...
The "ISIGMET" option pulls international SIGMETs from AviationWeather.gov. The feed uses dashed lines to separate each SIGMET. U.S. entries containing identifiers starting with `K` are shown first. Scroll with **X** and **Y** to move within a SIGMET and to advance to the next or previous report when reaching the end.
